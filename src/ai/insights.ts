...
function compute(): Insight[] {
  ...
  if (badAuto.length) out.push({ id: "auto", severity: "critical", title: `${badAuto.length} entegrasyon hatalı`, body: `${badAuto.map((a) => a.name).join(", ")}. Bekleyen kayıtlar senkronize olamıyor.`, pageIds: ["panel", "ayarlar"], actionLabel: "Yeniden bağla", actionHref: "/ayarlar/#entegrasyonlar", applyResult: `${badAuto.length} entegrasyon yeniden bağlandı.` });
